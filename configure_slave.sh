...
cat <<DELIMITER  > /etc/hadoop/conf.unicarbkb/health_check.sh
#!/bin/bash
...
chown -R hadoop:hadoop /etc/hadoop/conf.unicarbkb/health_check.sh
chmod ugo+x /etc/hadoop/conf.unicarbkb/health_check.sh
